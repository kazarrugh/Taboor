// Ready translated locale messages
export const messages = {
  en: {
    buttons: {
      near: "Search in Nearby Locations",
      metres: "{value} Meters",
      nextnumber: "Next Number",
      getnumber: "Get Number",
      cancelnumber: "Cancel Number",
      print: "Print",
      ok: "Ok",
      cancel: "Cancel",
      back: "Back",
      save: "Save",
      addButton: "Add",
      register: "Register New Branch",
      changecord: "Edit Coordinates",
      submitreview: "Submit Review",
      map: "Map",
      doneserving: "Finished Serving",
      skipnumber: "Skip Number that was not served",
      edittranslation: "Edit Translation",
      show: "Active",
      hide: "Hidden",
      skipcontact: "Skip Contact",
    },
    nav: {
      home: "Home",
      serve: "Serve",
      newbranch: "New Branch",
      display: "Display",
      signin: "Sign In",
      signout: "Sign Out",
      qrpaper: "QR Print",
      qrscan: "QR Scan",
      registration: "Registration Form",
    },
    select: {
      typeofserviceneeded: "Type Of Service Requested",
      typeofservice: "Type Of Service Provided",
      windownumberselect: "Window Number of Service Provider",
      windownumber: "Window Number ",
    },
    alerts: {
      currentnumber: "Current Number {value}",
      distance: "The user distance is {value} meters",
      servicerating: "Service Reviews",
      totalnumbers: "Total in Line",
      pendingnumbers: "Number Pending",
      currentlyserved: "Numbers Being Served",
      mynumberinline: "My Number in Line",
      areyousure: "Are you sure?",
      offline: "You are not connected to the internet",
      closedtomorrow:
        "Your number in line is set for tomorrow because it is now after business hours",
      resetemail: "You will receive an Email to reset password",
      nomorepeople: "No more people in line to serve.",
      savecompleted: "Save Completed Successfully",
      estimatedtime: "Estimated waiting time is ",
      peopleahead: "People before me in line ",
      ratingrequired: "Star rating is required",
      locationerror: "You did not allow location permision",
    },
    text: {
      fullname: "Type Your Full Name",
      usecamera: "Scan QR Code with your camera to get in line",
      service: "Service",
      forgotpassword: "Forgot Password",
      email_sign_in: "Type Email to Sign in",
      email_branch: "Branch Email Address",
      password: "Type Password ",
      passwordmin: "Password must contain at least {value} characters",
      passwordchange: "Edit Password",
      oldpassword: "Your Old Password",
      confirmpassword: "Confirm Password",
      confirmleave: "Leave Confirmation",
      leaveask: "Do you really want to leave? you have unsaved changes!",
      branch_name: "Branch name of service provider",
      profile: "Branch Profile",
      servicewindow: "Each type of service has a que",
      timenotset: "Time Not Set",
      maplocation: "Location on Map",
      howexperince: "How was your experince with {value}",
      lastupdated: "Last updated ",
      seconds: "seconds",
      or: "Or ",
      showtopublic: "Show {value} to public",
      contactinfo: "My Contact Information",
      choosetoshare: "Sharing your contact is not required",
    },
    labels: {
      name: "Name: ",
      email: "Email Address:",
      password: "Password:",
      curpassword: "Current Password",
      newpassword: "New Password",
      branch: "Branch Name:",
      phone: "Phone Number:",
      servicetype: "Type of activity:",
      servicewindow: "Types of Services:",
      opentime: "Open Time:",
      closetime: "Closing Time:",
      opendays: "Working Days:",
      logo: "Logo: ",
      window: "Window:",
      number: "Number:",
      review: "Rating:",
      comment: "Comment:",
      sponsors: "Sponsors",
    },
    days: {
      6: "Saturday",
      7: "Sunday",
      1: "Monday",
      2: "Tuesday",
      3: "Wednesday",
      4: "Thursday",
      5: "Friday",
    },
  },

  // ANOTHER LANGUAGE

  "ar-ly": {
    buttons: {
      near: "االبحث في نطاق قريب",
      metres: "{value} متر",
      nextnumber: "الرقم التالي",
      getnumber: "طلب رقم",
      cancelnumber: "الغاء الرقم",
      print: "طباعة",
      ok: "موافق",
      cancel: "الغاء الأمر",
      back: "الرجوع",
      save: "تخزين",
      addButton: "إضافة",
      register: "تسجيل فرع جديد",
      changecord: "تغيير الاحداثيات",
      submitreview: "ارسال التقييم",
      map: "الخريطة",
      doneserving: "انهاء العمل",
      skipnumber: "تخطي الرقم لعدم تواجده",
      edittranslation: "تعديل الترجمة",
      show: "مفعلة",
      hide: "مخفية",
      skipcontact: "تخطى التواصل",
    },
    nav: {
      home: "الرئيسية",
      serve: "خدمة",
      newbranch: "فرع جديد",
      display: "شاشة الآرقام",
      signin: "تسجيل الدخول",
      signout: "تسجيل الخروج",
      qrpaper: "QR طباعة",
      qrscan: "QR كاميرا",
      registration: "نموذج التسجيل",
    },
    select: {
      typeofserviceneeded: "نوع الخدمة المطلوبة",
      typeofservice: "نوع الخدمة المقدمة",
      windownumberselect: "رقم شباك مقدم الخدمة",
      windownumber: " شباك رقم ",
    },
    alerts: {
      currentnumber: "الرقم الحالي {value}",
      distance: "يبعد المستخدم مسافة {value} متر",
      servicerating: "تقييم الخدمات",
      totalnumbers: "كل ارقام الطابور",
      pendingnumbers: "الارقام في الانتظار",
      currentlyserved: "الارقام في الخدمة",
      mynumberinline: "رقمي في الطابور",
      areyousure: "هل انت متأكد؟",
      offline: "لا يوجد اتصال بشبكة الانترنت",
      closedtomorrow: "رقمك في الطابور سيكون ليوم الغد لإن ساعات الدوام انتهت",
      resetemail: "تم ارسال بريد الكتروني لتدوين كلمة مرور جديدة",
      nomorepeople: "لا يوجد اشخاص في الطابور",
      savecompleted: "تم التخزين بنجاح",
      estimatedtime: "وقت الانتظار التقديري  ",
      peopleahead: " عدد الاشخاص قبلي في طابور",
      ratingrequired: "التقييم بالنجوم اجباري",
      locationerror: "لم تتم الموافقة على خدمات الموقع",
    },
    text: {
      fullname: "اسمك الثلاتي",
      usecamera: "صور الرمز بالكاميرا لطلب طابور",
      service: "خدمة",
      forgotpassword: "نسيت كلمة المرور",
      email_sign_in: "البريد الإلكتروني لتسجيل الدخول",
      email_branch: "البريد الإلكتروني للفرع",
      password: "كلمة المرور",
      passwordmin: "كلمة مرور تتكون من اكثر من {value} حروف او ارقام",
      oldpassword: "كلمة المرور القديمة",
      confirmpassword: "تأكيد كلمة المرور",
      confirmleave: "تأكيد المغادرة",
      leaveask: "هل انت متأكد من مغادرة الصفحة؟ لم يتم تخزين التعديلات",
      passwordchange: "تغيير كلمة المرور",
      branch_name: "اسم فرع موزع الخدمة او الشركة",
      profile: "ملف تعريف الفرع",
      servicewindow: "   كل نوع خدمة له طابور خاص",
      timenotset: "لم يتم تحديد الوقت",
      maplocation: "الموقع في الخريطة",
      howexperince: "كيف كانت تجربتك في {value}",
      lastupdated: "أخر تحديث",
      or: "او ",
      seconds: "ثانية",
      showtopublic: "اظهار {value} للعامة",
      contactinfo: "بيانات التواصل معي",
      choosetoshare: "مشاركة بيانات التواصل غير اجباري",
    },
    labels: {
      name: "الإسم:",
      email: "البريد الإلكتروني:",
      password: "كلمة المرور:",
      curpassword: "كلمة المرور الحالية:",
      newpassword: "كلمة المرور الجديدة:",
      branch: "اسم الفرع:",
      phone: "رقم الهاتف:",
      servicetype: "نوع النشاط:",
      servicewindow: "انواع الخدمات:",
      opentime: "بداية ساعات العمل:",
      closetime: "نهاية ساعات العمل:",
      opendays: "ايام العمل:",
      logo: "الشعار:",
      window: "شباك:",
      number: "رقم:",
      review: "التقييم: ",
      comment: "التعليق",
      sponsors: "برعاية",
    },
    days: {
      6: "السبت",
      7: "الأحد",
      1: "الأثنين",
      2: "الثلاتاء",
      3: "الإربعاء",
      4: "الخميس",
      5: "الجمعة",
    },
  },
};
